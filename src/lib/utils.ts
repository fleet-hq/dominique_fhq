 ...
 *
 * ``tz`` is the booking's tenant timezone. Pass it whenever the
 * input represents a booking moment (pickup / dropoff / created_at)
 * so the render shows the rental-location wall-clock, not the
 * customer's browser. Without ``tz`` we fall through to the legacy
