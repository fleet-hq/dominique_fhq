...
export default function EditCheckoutPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setBooking(data);
        if (!data.timezone) {
          setError("Booking is missing a timezone.");
          return;
        }

        // Convert customer-typed wall-clock to UTC using the
        // BOOKING'S tenant timezone (not the customer's browser).
        // The old ``${date}T${time}:00`` concat was treated as UTC
        // by the backend and silently shifted by the customer's
        // browser offset.
        const newPickup = toUtcIso(
          newPickupDate,
          newPickupTime || "00:00",
          data.timezone,
        );
        const newDropoff = toUtcIso(
          newDropoffDate,
          newDropoffTime || "00:00",
          data.timezone,
        );
        const isExtension =
          new Date(newDropoff).getTime() >
          new Date(data.dropOff.rawDatetime).getTime();
  ...
  const handlePay = async () => {
    ...
    const isExtension =
      new Date(newDropoff).getTime() >
      new Date(booking.dropOff.rawDatetime).getTime();
