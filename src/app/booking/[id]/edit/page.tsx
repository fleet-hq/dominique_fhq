...
export default function EditTripPage() {
  ...
  const fleetDateArgs = useMemo(() => {
    ...
    if (!booking?.timezone) return undefined;
    return {
      pickupDatetime: toUtcIso(
        debouncedPickup.date,
        debouncedPickup.time || "00:00",
        booking.timezone,
      ),
      dropoffDatetime: toUtcIso(
        debouncedDropoff.date,
        debouncedDropoff.time || "00:00",
        booking.timezone,
      ),
    };
  }, [debouncedPickup, debouncedDropoff, booking?.timezone]);
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setBooking(data);
        // Pre-fill the form in the booking's tenant TZ — the stored
        // datetime is UTC, so naively splitting on "T" would show a
        // customer who booked 11 PM EDT the literal "03:00" the next
        // morning (the UTC time), not the 23:00 they originally typed.
        if (data.timezone && data.pickUp.rawDatetime) {
          const p = utcIsoToFormValues(data.pickUp.rawDatetime, data.timezone);
          setPickupDate(p.date);
          setPickupTime(p.time);
        } else if (data.pickUp.rawDatetime) {
          // No tenant TZ available — fall back to UTC literals.
        ...
        if (data.timezone && data.dropOff.rawDatetime) {
          const d = utcIsoToFormValues(data.dropOff.rawDatetime, data.timezone);
          setDropoffDate(d.date);
          setDropoffTime(d.time);
        } else if (data.dropOff.rawDatetime) {
  ...
  useEffect(() => {
    ...
    if (!booking.timezone) return; // no tenant TZ → can't safely interpret input

    // Interpret the customer's typed wall-clock in the booking's
    // tenant timezone, then serialise to UTC. Replaces the old
    // ``${date}T${time}:00`` naive concat which the backend used
    // to treat as UTC.
    const newPickup = toUtcIso(pickupDate, pickupTime, booking.timezone);
    const newDropoff = toUtcIso(dropoffDate, dropoffTime, booking.timezone);
