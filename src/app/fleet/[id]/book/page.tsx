...
function BookingPageContent() {
  ...
  const fleetDateArgs = (() => {
    ...
    if (!branch?.timezone) return undefined;
    return {
      pickupDatetime: toUtcIso(
        debouncedPickupDate,
        debouncedPickupTime || "00:00",
        branch.timezone,
      ),
      dropoffDatetime: toUtcIso(
        debouncedDropoffDate,
        debouncedDropoffTime || "00:00",
        branch.timezone,
      ),
    };
  })();
  ...
  const insuranceFetchArgs = (() => {
    ...
    if (!branch?.timezone) return undefined;
    const pickup = toUtcIso(
      initialPickupDate,
      initialPickupTime || "00:00",
      branch.timezone,
    );
    const dropoff = toUtcIso(
      initialDropoffDate,
      initialDropoffTime || "00:00",
      branch.timezone,
    );
  ...
  // Legacy two-step mutations — kept imported so the rollback is trivial.
  // The submit handler now uses startCheckoutMutation instead.
  const { mutate: createBookingMutation } = useCreateBooking();
  void createBookingMutation;
  void useCreateCheckoutSession;

  // NEW pre-payment flow: single call that validates input, places an
  // inventory hold, creates a Stripe Checkout session, and returns the URL.
  // No Booking row exists until Stripe confirms payment (see /booking/success).
  const { mutate: startCheckoutMutation, isPending: isStartingCheckout } = useStartBookingCheckout();
  ...
  const handleReserve = async () => {
    ...
    // Build pickup and dropoff datetime as canonical UTC ISO,
    // interpreting the customer-typed wall-clock in the rental
    // location's timezone (not the customer's browser). Without
    // this, a renter in Mumbai picking "10 AM" for a NY tenant
    // would have the backend interpret it as 10 AM UTC and store
    // 6 AM EDT — wrong by the browser→tenant offset.
    const tenantTz = branch?.timezone;
    if (!tenantTz) {
      setValidationModal({
        isOpen: true,
        title: "Missing Timezone",
        message:
          "We couldn't determine the rental location's timezone. Please refresh and try again.",
      });
      return;
    }
    const pickupDatetime = toUtcIso(pickupDate, pickupTime, tenantTz);
    const dropoffDatetime = toUtcIso(dropoffDate, dropoffTime, tenantTz);
    ...
    // NEW single-call flow: validates + holds inventory + creates Stripe
    // Checkout session in one call. The Booking is only materialised after
    // Stripe confirms payment — the success page polls /by-session/.
    ...
    startCheckoutMutation(
      {
        ...
        cancel_url: `${origin}/fleet/${vehicleId}/book`,
      },
      {
        onSuccess: (data) => {
        ...
        onError: (error: unknown) => {
          // DRF errors come back as { field: ["msg"] } or { errors: { field: ["msg"] } }
