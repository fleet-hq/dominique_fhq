...
function FleetPageContent() {
  ...
  // Tenant timezone for interpreting the customer-typed search dates.
  // Without it, the availability query would send the customer's
  // browser-local wall-clock as if it were UTC.
  ...
  const pickupDatetime = useMemo(() => {
    ...
    if (!date || !tenantTz) return null;
    return toUtcIso(date, time || '10:00', tenantTz);
  }, [searchParams, tenantTz]);
