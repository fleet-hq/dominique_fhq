...
export interface AgreementData {
   ...
   *  day instead of the customer's browser-local one. */
  timezone?: string | null;
  company: {
    name: string;
    address: string;
...
function transformAgreement(api: ApiAgreement): AgreementData {
  ...
  return {
    ...
    timezone:
      (booking as any)?.timezone ||
      (booking as any)?.pickup_location?.timezone ||
      company?.timezone ||
      null,
    company: {
    ...
    vehicle: {
      pickupDateTime: formatDateTime(
        booking?.pickup_datetime || null,
        (booking as any)?.timezone ||
          (booking as any)?.pickup_location?.timezone ||
          company?.timezone,
      ),
      dropoffDateTime: formatDateTime(
        booking?.dropoff_datetime || null,
        (booking as any)?.timezone ||
          (booking as any)?.pickup_location?.timezone ||
          company?.timezone,
      ),
